...
// REMAINING TEST SCENARIOS FROM PYTHON
...
#[test]
fn test_multi_intersection_single_point() {
    let current_state = create_test_batch(vec![
...
#[test]
fn test_multi_intersection_multiple_point() {
    let current_state = create_test_batch(vec![
...
fn test_extend_update() {
    ...
    let expected = ExpectedResult {
        ...
        insert: vec![
            // Should be conflated to extend backwards
...
#[test]
fn test_no_change_with_intersection() {
    let current_state = create_test_batch(vec![
        (1234, "test", 100, 100, "2020-01-01", "max", "2025-01-01", "max"),
    ]).unwrap();
...
// Now add the manual Python test scenarios
#[test]
